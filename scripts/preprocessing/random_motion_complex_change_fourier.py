...
class RandomMotion(RandomTransform, IntensityTransform):
    ...
    def apply_transform(self, subject: Subject) -> Subject:
        ...
        return self.add_include_exclude(arguments)
...
class Motion(IntensityTransform):
    ...
    def fourier_transform(tensor: torch.Tensor) -> torch.Tensor:
        try:
            ...
            a = torch.fft.ifftshift(tensor)
            a = torch.fft.fftn(a, norm="ortho")
            a = torch.fft.fftshift(a)
            return a
        except (ModuleNotFoundError, AttributeError):
            import torch

            a = np.fft.ifftshift(tensor)
            a = np.fft.fftn(a, norm="ortho")
            a = np.fft.fftshift(a)
    ...
    def inv_fourier_transform(tensor: torch.Tensor) -> torch.Tensor:
        try:
            ...
            a = torch.fft.ifftshift(tensor)
            a = torch.fft.ifftn(a, norm="ortho")
            a = torch.fft.fftshift(a)
            return a
        except (ModuleNotFoundError, AttributeError):
            import torch
