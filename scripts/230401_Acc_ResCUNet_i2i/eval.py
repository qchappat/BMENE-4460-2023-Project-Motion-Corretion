...
def eval(model: Module, testloader: DataLoader, save_dir: Path, args) -> pd.DataFrame:
    ...
    with PdfPages(save_dir / 'eval.pdf') as pdf, tqdm(total=len(testloader), dynamic_ncols=True) as pbar, torch.no_grad():
        for kspace, ispace, original_kspace, original_ispace in testloader:
            ...
            pred_ispace = list()
            for slc_idx in range(ispace.shape[0]):
                pred_ispace.append(model(ispace[slc_idx:slc_idx + 1]))
            pred_ispace = torch.cat(pred_ispace)

            img = original_ispace.abs().mean(dim=1).cpu().numpy()
            corrupted_img = ispace.abs().mean(dim=1).cpu().numpy()
            pred_img = pred_ispace.abs().mean(dim=1).cpu().numpy()

            img /= img.max()
            corrupted_img /= corrupted_img.max()
            pred_img /= pred_img.max()
            ...
            fig = plot(img, corrupted_img, pred_img, input_metrics, pred_metrics)
