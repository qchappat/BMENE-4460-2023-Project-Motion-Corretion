...
from models import ResCUNet as Model
...
def train_epoch(model: Module, trainloader: DataLoader, loss_func, optimizer, args):
    ...
    for idx, (kspace, ispace, original_kspace, original_ispace) in enumerate(trainloader):
        original_ispace = original_ispace.to(args.device)
        ispace = ispace.to(args.device)
        predicted_ispace = model(ispace)

        loss = loss_func(
            predicted_ispace.abs().mean(dim=1),
            original_ispace.abs().mean(dim=1))
...
def val_epoch(model: Module, valloader: DataLoader, args):
    ...
    with tqdm(total=len(valloader), dynamic_ncols=True) as pbar, torch.no_grad():
        for kspace, ispace, original_kspace, original_ispace in valloader:
            ...
            img = original_ispace.abs().mean(dim=1).detach().cpu().numpy()
            corrupted_img = ispace.abs().mean(dim=1).detach().cpu().numpy()
            predicted_img = predicted_ispace.abs().mean(dim=1).detach().cpu().numpy()
