...
from dataset import AccSliceDataset, AccVolumeDataset
from logger import Logger
from models import ResCUNet_AFT_ResCUNet as Model
...
def setup_dataloader(args) -> tuple[DataLoader, DataLoader]:
    ...
    trainloader = torch.utils.data.DataLoader(
        AccSliceDataset(args.data_csv, mode='multicoil_train', corresp_dict=corresp_dict),
        batch_size=1, shuffle=True, num_workers=1)
    valloader = torch.utils.data.DataLoader(
        AccVolumeDataset(args.data_csv, mode='multicoil_val', corresp_dict=corresp_dict),
...
def setup_model(save_dir: Path, args) -> Module:
    ...
    if args.resume:
        ...
        model.rescunet2.load_state_dict(torch.load(args.ResCUNet))
...
def train_epoch(model: Module, trainloader: DataLoader, loss_func, optimizer, args):
    ...
    for idx, (kspace, ispace, original_kspace, original_ispace) in enumerate(trainloader):
        pred_ispace = model(kspace.to(args.device))

        loss = loss_func(
            pred_ispace.abs().mean(dim=1),
            original_ispace.to(args.device).abs().mean(dim=1)
        )
...
def val_epoch(model: Module, valloader: DataLoader, args):
    ...
    with tqdm(total=len(valloader), dynamic_ncols=True) as pbar, torch.no_grad():
        for kspace, ispace, original_kspace, original_ispace in valloader:
            pred_ispace = torch.cat([model(slc.unsqueeze(0).to(args.device)) for slc in kspace])

            img = original_ispace.abs().mean(dim=1).cpu().numpy()
            ...
            input_ssim.append(
                metrics.structural_similarity(img, corrupted_img, data_range=1)
            )
            pred_ssim.append(
                metrics.structural_similarity(img, predicted_img, data_range=1)
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--AFT', required=True, help='saved AFT weights')
    parser.add_argument('--ResCUNet', required=True, help='saved ResCUNet weights')
    ...
    for epoch in range(1, 1000):
        ...
        print(f'GPU_memory_usage = {get_memory_usage()}')
